...
var F = function (opts) {
  ...
  this._getWidgets().bind('change:autoStyle', this._onAutoStyleChange, this);
  this._getWidgets().bind('change', _.debounce(saveStateDefinition, 500), this);
...
F.prototype._getWidgets = function () {
  return this._diDashboard._dashboard.widgets._widgetsCollection;
};
...
F.prototype._onAutoStyleChange = function (m, autoStyle) {
  var layer = m.dataviewModel.layer;
  if (layer != null) {
    if (autoStyle) {
      layer.disableLegends();
    } else {
      layer.enableLegends();
    }
  }
